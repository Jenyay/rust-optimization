...
fn create_optimizer<'a>(
    ...
    let inertia = Box::new(speedcalc::ConstInertia::new(0.85));
    // let inertia = Box::new(speedcalc::LinearInertia::new(0.2, 0.9, 400));

    // Speed calculator
    let speed_calculator = speedcalc::InertiaSpeedCalculator::new(
        phi_personal,
        phi_global,
        inertia
    );
