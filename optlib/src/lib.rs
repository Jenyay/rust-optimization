//! # optlib
//! A crate for global optimization algorithms.
//! A crate uses common traits for easy switch between algorithms.
extern crate num;
...
/// Trait for single point in search space. The trait used with `AlgorithmWithAgents`.
/// `T` - type of a point in search space for goal function.
pub trait Agent<T> {
